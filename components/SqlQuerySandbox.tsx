...
export default function SqlQuerySandbox() {
  ...
  return (
    <div className="my-10 p-6 sm:p-8 bg-[#1a1a1a] text-[#f7f5f0] border border-black/20 rounded-[2px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-[#3a3530]">
        <div>
          <span className="text-xs font-semibold text-[var(--color-gold)] uppercase tracking-[0.12em] flex items-center gap-2">
            <Terminal className="w-3.5 h-3.5" />
            <span>Interactive SQL Analytics Sandbox</span>
          </span>
          <h4 className="font-serif text-lg text-[var(--color-bg)] mt-0.5">
            PostgreSQL Query Performance & Analytical Pipeline
          </h4>
        </div>
        <div className="flex items-center gap-2 text-xs font-sans text-[#a8a296]">
          <Database className="w-3.5 h-3.5 text-[var(--color-gold)]" />
          <span>PostgreSQL v16 Engine</span>
        </div>
      </div>

      {/* Query Selector Tabs */}
      <div className="flex flex-wrap gap-2 mb-6 border-b border-[#3a3530] pb-3">
        {queries.map((q) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2 text-xs font-sans rounded-[2px] transition-colors cursor-pointer flex items-center gap-2 ${
                isActive
      ...
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
        <div>
          <h5 className="font-serif text-base text-[var(--color-bg)]">
            {activeQuery.title}
          </h5>
          <p className="text-[#a8a296] text-xs leading-relaxed mt-0.5">
            {activeQuery.purpose}
          </p>
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="bg-[#0f0f0f] border border-[#3a3530] rounded-[2px] p-4 font-mono text-xs text-[#d8d3c8] overflow-x-auto leading-relaxed shadow-inner"
        >
          <pre>
